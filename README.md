...
- [Bugs](#bugs)
	...
	- [Calling fw.saveDocument() via MMExecute() throws an exception if the file doesn’t already exist](#calling-fwsavedocument-via-mmexecute-throws-an-exception-if-the-file-doesnt-already-exist)
	- [Rich symbol scripts get called multiple times unnecessarily](#rich-symbol-scripts-get-called-multiple-times-unnecessarily)
	- [dom.moveSelectionTo() doesn’t move gradient handles of elements inside groups](#dommoveselectionto-doesnt-move-gradient-handles-of-elements-inside-groups)
	- [Auto shape icons in the toolbox and Shapes panel get mixed up if auto shapes have the same name](#auto-shape-icons-in-the-toolbox-and-shapes-panel-get-mixed-up-if-auto-shapes-have-the-same-name)
- [Undocumented features](#undocumented-features)
	...
	- [fw.getDocumentDOM() can access the DOM inside a symbol](#fwgetdocumentdom-can-access-the-dom-inside-a-symbol)
